...
const ORBS: Orb[] = SPECIALTIES.map((s, i) => ({
  ...
  x: 3 + seeded(i, 1) * 94,
  y: 4 + seeded(i, 2) * 90,
  size: 7 + seeded(i, 3) * 14,
  depth: 0.4 + seeded(i, 4) * 0.8,
  delay: seeded(i, 5) * 6,
  dur: 7 + seeded(i, 6) * 6,
}));
  ...
  return (
    <motion.div className="absolute" style={{ left: `${o.x}%`, top: `${o.y}%`, x, y }}>
      <div className={cn(!reduce && "animate-float")} style={{ animationDelay: `${o.delay}s`, animationDuration: `${o.dur}s` }}>
        ...
        >
          {o.featured && !reduce && (
            <span className="absolute inset-0 animate-ping-soft rounded-full" style={{ background: color, animationDelay: `${o.delay}s` }} />
          )}
        </span>
      </div>
    </motion.div>
